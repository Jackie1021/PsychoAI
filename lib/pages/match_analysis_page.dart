  ...
  Future<void> _startChat(BuildContext context) async {
    ...
    try {
      ...
      final conversationId = await chatProvider.createOrGetConversation(
        otherUserId: analysis.userB.uid,
        matchId: analysis.id,
      );
      ...
      // Wait for the provider to include the new conversation so ChatPage can load reliably
      Future<void> _waitForConversation(ChatProvider provider, String id,
          {int timeoutMs = 5000, int pollMs = 200}) async {
        final stopwatch = Stopwatch()..start();
        while (provider.getConversationById(id) == null) {
          if (stopwatch.elapsedMilliseconds > timeoutMs) break;
          await Future.delayed(Duration(milliseconds: pollMs));
        }
        if (provider.getConversationById(id) == null) {
          throw Exception('Timed out waiting for conversation to appear');
        }
      }
