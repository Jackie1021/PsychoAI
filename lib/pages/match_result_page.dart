...
import 'package:flutter_app/pages/chat_history_page_new.dart';
...
class _MatchResultPageState extends State<MatchResultPage> {
  ...
  void initState() {
    ...
    if (widget.useCachedResults) {
      ...
      _matchesFuture = apiService.getMatches(uid).then((matches) {
        // Automatically save all new match records
  ...
  Future<void> _saveMatchRecords(
      List<MatchAnalysis> matches, String userId) async {
    try {
      final apiService = locator<ApiService>();
      ...
      print('✅ Saved ${matches.length} match records');
    } catch (e) {
  ...
  void _navigateToChatHistory() {
    Navigator.of(context).push(MaterialPageRoute(
      builder: (context) => const ChatHistoryPageNew(),
    ));
  }
        ...
        child: FutureBuilder<List<MatchAnalysis>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              print('❌ MatchResultPage FutureBuilder error: ${snapshot.error}');
              return Center(child: Text('Error: ${snapshot.error}'));
            } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
              print('⚠️ MatchResultPage: No matches data or empty list');
            ...
            print('✅ MatchResultPage: Received ${matches.length} matches');
            for (var i = 0; i < matches.length; i++) {
              print('   Match $i: id=${matches[i].id}, score=${matches[i].totalScore}, userB=${matches[i].userB.username}');
            }
...
class _BubbleClusterState extends State<BubbleCluster> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (scores.isEmpty || widget.matches.isEmpty) {
      print('⚠️ BubbleCluster: No matches to display');
      _metaballs = [];
    ...
    _metaballs = widget.matches.map((analysis) {
      ...
      // 处理scoreRange为0的情况
      final radius = scoreRange == 0
          ? (minRadius + maxRadius) / 2
          : minRadius + (maxRadius - minRadius) * (analysis.totalScore - minScore) / scoreRange;
      return Metaball(analysis, radius: radius);
    }).toList();

    _simulation = MetaballSimulation(metaballs: _metaballs, size: _clusterSize);
    
    print('✅ BubbleCluster initialized with ${_metaballs.length} bubbles');
    
    // 确保动画在首次构建后启动
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && _metaballs.isNotEmpty && !_controller.isAnimating) {
        _resetSimulation();
        _controller.repeat();
        print('✅ BubbleCluster animation started');
      }
    });
  }
  ...
  void _resetSimulation() {
    if (!mounted || _metaballs.isEmpty) return;
    if (_clusterSize.width <= 0 || _clusterSize.height <= 0) return;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final ball in metaballs) {
      ...
      final textPainter = TextPainter(
        ...
        maxLines: 2,
      )..layout(maxWidth: (scaledRadius * 1.6).clamp(10.0, double.infinity));
