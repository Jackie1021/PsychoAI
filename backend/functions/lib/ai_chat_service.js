...
exports.analyzeChatAndFindMatches = exports.sendAiChatMessage = exports.hasVipAccess = void 0;
 ...
 */
async function callGeminiForChat(apiKey, message, conversationHistory) {
 ...
 */
async function analyzeConversationForMatches(apiKey, conversationText, currentUser, allUsers) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;
...
`;
    ...
    try {
        ...
        const matchProfiles = result.matches.map((match) => {
        ...
        }).filter((profile) => profile.uid); // Only include valid matches
 ...
 */
exports.hasVipAccess = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
 ...
 */
exports.sendAiChatMessage = functions
    .runWith({
    timeoutSeconds: 60,
    memory: "512MB"
})
 ...
 */
exports.analyzeChatAndFindMatches = functions
    .runWith({
    timeoutSeconds: 120,
    memory: "1GB"
})
    ...
    try {
        ...
        const matches = await analyzeConversationForMatches(apiKey, conversationText, currentUser, allUsers);
...
//# sourceMappingURL=ai_chat_service.js.map