...
exports.analyzeYearlyPattern = exports.analyzeMatchPattern = void 0;
 ...
 */
exports.analyzeMatchPattern = functions
    .runWith({
    ...
    if (!userId || !statistics || !traitAnalysis || !dateRange) {
        throw new functions.https.HttpsError("invalid-argument", "Missing required fields: userId, statistics, traitAnalysis, dateRange");
    }
    try {
...
${traits
            .slice(0, 5)
            .map((t) => `- **${t.trait}**: 匹配${t.matchCount}次，平均分${(t.avgScore).toFixed(1)}，成功率${(t.successRate * 100).toFixed(1)}%`)
            .join("\n")}
 ...
 */
exports.analyzeYearlyPattern = functions
    .runWith({
    ...
    const { userId, statistics, traitAnalysis, chatSummaries, dateRange } = data;
    if (!userId ||
        !statistics ||
        !traitAnalysis ||
        !chatSummaries ||
...
${traits
            .slice(0, 5)
            .map((t, idx) => `${idx + 1}. **${t.trait}**: ${t.matchCount}次匹配，成功率${(t.successRate * 100).toFixed(1)}%`)
            .join("\n")}
...
- 必须返回有效的 JSON`;
        ...
        if (!parsedResponse.overallSummary ||
...
//# sourceMappingURL=llm_analysis_service.js.map